...
- Cleanup state is persisted to `RESTORE_STATE_FILE` so the last restore can be cleaned up even after a restart.
...
- `.env.example` — example environment variables, including `PORT` and `RESTORE_STATE_FILE`
...
- The app stores restore metadata to the configured `RESTORE_STATE_FILE`, which is optional but recommended for reliable cleanup.
